...
const ServerDetailsBlock = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={classNames('grid grid-cols-6 gap-2 md:gap-4', className)}>
            ...
            <StatBlock icon={faMicrochip} title={'Нагрузка процессора'} color={getBackgroundColor(stats.cpu, limits.cpu)}>
                {status === 'offline' ? (
            ...
            <StatBlock icon={faCloudDownloadAlt} title={'Входящий трафик'}>
                {status === 'offline' ? <span className={'text-gray-400'}>Оффлайн</span> : bytesToString(stats.rx)}
            </StatBlock>
            <StatBlock icon={faCloudUploadAlt} title={'Исходящий трафик'}>
