...
var DateFormat = {};
! function (a) {
    var b = ["Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье"],
        c = ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", 'Вс'],
        d = ["Янв", "Февр", "Мар", "Апр", "Май", "Июн", "Июл", "Авг", "Сент", "Окт", "Нояб", "Дек"],
        e = ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"],
        f = {
            Jan: "01",
            Feb: "02",
            Mar: "03",
    ...
    a.format = function () {
        ...
        return {
            ...
            prettyDate: function (a) {
                var b, c, d;
                return ("string" == typeof a || "number" == typeof a) && (b = new Date(a)), "object" == typeof a && (b = new Date(a.toString())), c = ((new Date).getTime() - b.getTime()) / 1e3, d = Math.floor(c / 86400), isNaN(d) || 0 > d ? void 0 : 60 > c ? "прямо сейчас" : 120 > c ? "1 минуту назад" : 3600 > c ? Math.floor(c / 60) + " минут назад" : 7200 > c ? "1 час назадo" : 86400 > c ? Math.floor(c / 3600) + " часов назад" : 1 === d ? "Вчера" : 7 > d ? d + " дней назад" : 31 > d ? Math.ceil(d / 7) + " недель назад" : d >= 31 ? "более 5 недель назад" : void 0
            },
            toBrowserTimeZone: function (a, b) {
                return this.date(new Date(a), b || "dd/MM/yyyy HH:mm:ss")
            }
        }
    }()
}(DateFormat),
